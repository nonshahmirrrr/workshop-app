...
def add_record():
    ...
    if request.method == "POST":
        ...
        # Reset AUTO_INCREMENT after inserting a new record
...
def edit_record(record_id):
    ...
    if request.method == "POST":
        ...
        # Reset AUTO_INCREMENT after updating a record
        cur.execute("SELECT MAX(RecordID) FROM Service_Record")
        max_id = cur.fetchone()[0]
        if max_id is None:  # If the table is empty
            cur.execute("ALTER TABLE Service_Record AUTO_INCREMENT = 1")
    ...
    v_opts = "".join([f'<option value="{v[0]}" {"selected" if v[0]==v_id else ""}>{escape(v[1] or "")}</option>' for v in vehicles_])
